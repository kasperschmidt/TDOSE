...
```
in a Python environment. Here `tdose_setup.txt` is a completed TDOSE setup file. A template setup file can be genererated with `tdose_utilities.generate_setup_template()`. Keywords to `tdose.perform_extraction()` can be used to ignore individual steps of the TDOSE run, which can be useful for repeated runs with minor changes to the setup. See the header of `tdose.perform_extraction()` for details on these keywords.
...
The following presents and overview and description of the outputs that can be generated with TDOSE. The generation of a number of these can be constrolled from the TDOSE setup file. In the list below the [`name`] refers to the TDOSE setup parameter defining the name extension/generation of the output.

- Reference Image Model [`model_image_ext`]
  - Model of the reference image (cutout). The reference image defines the high resolution image to define the morphology of the object before convolution with the data cybe PSF. This model is generated by fitting multi-variate Guassians to the sources if `source_model=gauss` in the TDOSE setup file.
- Source Model Parameters [`model_param_reg`]; First Table
...
- Image Model in Cube "WCS frame"  [`model_image_cube_ext`]; 2D Fits Image
  - Image showing the model in the "WCS frame" of the data cube to extract spectra from, i.e., converting the pixel scale of the high-resolution reference image model.
- PSF Cube [`psf_savecube`]; 3D Fits Cube
  - Cube witht he modeled PSF spatially centered capturing the wavelength evolution of the (Gaussian) model for the intrumt PSF defined in the TDOSE setup file. The extension of the this cube will be `*_psfcube_[source_model].fits` by default.
...
- Source Model Cube [`source_model_cube_ext`]; 4D Fits Cube
  - Data cube containing the de-blended source models for all sources in the field-of-view modeled, i.e. the dimensions are (source number, wavelength, y-axis, x-axis). This cube can be used to modify the data cube by removing individual sources (i.e. source models) with `tdose_modify_cube.py`.
- Spectrum [`spec1D_name`]; Fits Table
  - Stored 1D spectrum of individual objects. An object spectrum can be an arbitrariy combination of N sources. However, by default each source corresponds to a different object and the number of sources therefore equals the number of extracted object spectra.
